...
- ⚠️ **GFN2-xTB (Native)** - AI-implemented, machine-tested; canonical `gfn2` backend; `-opt` works; Broyden SCF default (`-scf_mode diis|plain|level-shift`, `-scf_guess h0|eeq`); 11/12 sqm_reference @1e-8 vs tblite (only 231-atom `complex` open) — [docs/SQM_VALIDATION.md](docs/SQM_VALIDATION.md), [docs/SCF_MODES.md](docs/SCF_MODES.md)
  - **d-shell**: main-group d via cartesian→spherical transform on CPU + CUDA + ROCm (~1e-8 Eh vs tblite); Vulkan falls back to CPU; transition metals enabled but unvalidated — [docs/SQM_DSHELL_WP.md](docs/SQM_DSHELL_WP.md)
  ...
  - **Integral setup**: shell-pair-blocked overlap/multipole/gradient kernels (setup 209→91 ms, gfn2 total 1199→1083 ms on complex/231) — [docs/SQM_PERFORMANCE.md](docs/SQM_PERFORMANCE.md)
  ...
  - **CUDA is a runtime `dlopen` plugin** (`libcurcuma_cuda.so`): the main binary is CUDA-free so CPU-only runs start in ~9 ms; `-gpu cuda` loads it lazily — [docs/GPU_PLUGIN_STARTUP.md](docs/GPU_PLUGIN_STARTUP.md)
...
- ⚠️ **Native GFN-FF** - AI-implemented, machine-tested; S30L validation history + residuals in [docs/S30L_GFNNF_VALIDATION.md](docs/S30L_GFNNF_VALIDATION.md); status in [docs/GFNFF_STATUS.md](docs/GFNFF_STATUS.md)
...
- ⚠️ **Native GFN1/GFN2 ALPB + GBSA** - self-consistent ALPB (P16) / GBSA (Still) in the native xTB SCF, matching tblite total ΔG to ≤1e-8 Eh (CPU + GPU); CPCM still pending — [docs/SQM_SOLVATION_WP.md](docs/SQM_SOLVATION_WP.md)
- ⚠️ **Native GFN-FF ALPB** - Born reaction field couples into the EEQ solve (`A_eeq += B`); matches xtb 6.7.1 to ≤1e-8 Eh; `-gfnff.solvent_model gbsa` maps to ALPB — [docs/SQM_SOLVATION_WP.md](docs/SQM_SOLVATION_WP.md) WP5
...
- **ConfSearch** - Systematic conformational searching (unified trajectory framework): **dual-method** runs, `-restart` checkpoints, **registry-backed** (67 PARAMs, shared `ChildConfig()`), and RMSD-MTD bias via Gaussian-cutoff screen (`-rmsd_mtd_screen`, ON) + pool cap (`-rmsd_mtd_max_gaussians`) — see [docs/CONFSEARCH_MTD_SCREEN.md](docs/CONFSEARCH_MTD_SCREEN.md), [docs/CONFSEARCH_DUAL_METHOD.md](docs/CONFSEARCH_DUAL_METHOD.md), [docs/CONFSEARCH_RESTART.md](docs/CONFSEARCH_RESTART.md)
...
- ⚠️ **Temperature ramps / live T / thermal regions** - live `setTargetTemperature()`, `temp_ramp`/`temp_schedule` (`steps`/`reach` modes), per-atom-subset `temp_regions` (Berendsen/CSVR/Andersen) — [docs/TEMPERATURE_RAMP.md](docs/TEMPERATURE_RAMP.md)
...
> Older 2025 work (parameter registry, polymorphic EnergyCalculator, native GFN2/GFN1/PM3, MNDO integrals, GFN-FF full implementation, scattering, analysis parallelization, dependency gating) is in `AIChangelog.md` + git history. June–July 2026 milestones are archived in `claudemd-archive-0826.txt`.
...
- **Archive (pre-Sept-2026)**: Historical, dated entries removed from this file are kept in [claudemd-archive-0826.txt](claudemd-archive-0826.txt)
...
2. **GFN-FF S30L validation (Jul 2026)**: fix history archived in `claudemd-archive-0826.txt`. Current state: 28/30 within ~1.5 kcal/mol vs xtb 6.6.1 (MAD 433→1.38); open residuals: 23 (.CHRG quirk, curcuma correct), 27/28+7/8 (torsion deficit), 30/AB (bond pibo) — see [docs/S30L_GFNNF_VALIDATION.md](docs/S30L_GFNNF_VALIDATION.md)
...
5. **Native GFN transition metals fixed (MOR41, Jul 2026)**: native GFN1/GFN2 reproduce tblite for all 95 MOR41 structures (<1e-6 Eh; reaction MAD 0.03 kcal/mol). Fix diary archived in `claudemd-archive-0826.txt` — see [docs/MOR41_VALIDATION.md](docs/MOR41_VALIDATION.md)

6. **GFN-FF metal-coordinated aromatics + reference split (Jul 2026)**: all FT-HMO / carbene / halogen-bond / EEQ-array / torsion fixes ported; the port tracks pprcht/gfnff to per-structure MAD 0.004, reaction MAD 0.008 (all 95 within 0.1 kcal). Fix diary + reference-split discussion (GFN-FF ~63 kcal from DLPNO → unsuitable for MOR41 thermochemistry) archived in `claudemd-archive-0826.txt` — see [docs/MOR41_VALIDATION.md](docs/MOR41_VALIDATION.md)
